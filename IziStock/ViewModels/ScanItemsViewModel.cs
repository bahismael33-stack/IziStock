            ...
            if (!string.IsNullOrWhiteSpace(BarcodeNumber))
            {
                ...
                if (item != null)
                {
                    if (item.Quantity <= 0)
                    {
                        ...
                        ItemPurchaseStatusMessage = "Il ne reste plus d'articles disponibles à l'achat pour cet article ! Code-barres : " + BarcodeNumber;
                        PurchaseInfoIsVisible = false;
                        // play failure sound
                    ...
                    else
                    {
                        ...
                        ItemPurchaseStatusMessage = "Article trouvé et acheté avec succès ! Code-barres : " + BarcodeNumber;
                        PurchasedItem = item;
                ...
                {
                    ItemPurchaseStatusBrush = new SolidColorBrush(Colors.Red);
                    ItemPurchaseStatusMessage = "Article introuvable ! Code-barres : " + BarcodeNumber;
                    PurchaseInfoIsVisible = false;
                    // play failure sound
                    _failureSoundPlayer.Play();
                }
        ...
        private void SavePurchaseUpdatesToDB()
        {
            if (PurchaseInfoIsVisible && PurchaseInfo != null)
            {
                ...
                if (Quantity > PurchasedItem.Quantity)
                {
                    ...
                    _failureSoundPlayer.Play();
                    QuantityErrorMessage = "La quantité à acheter est supérieure au nombre d'articles disponibles";
                }
                else
                {
                    QuantityErrorMessage = "";
