...
namespace IziStock.Helpers
{
    class StockInfoExcelGenerator
    {
        ...
        public void ExportStockInfo(List<DetailedStockReportInfo> items, DateTime startDate, DateTime endDate, string path)
        {
            ...
            {
                var worksheet = workbook.Worksheets.Add("Infos Stock");
                worksheet.Cell("A1").Value = "IziStock -- Rapport de stock des articles vendus";
                worksheet.Cell("A1").Style.Font.Bold = true;
                worksheet.Cell("A2").Value = startDateString + " - " + endDateString;

                // table headers
                worksheet.Cell("A4").SetValue("Nom").Style.Font.SetBold(true);
                worksheet.Cell("B4").SetValue("Description").Style.Font.SetBold(true);
                worksheet.Cell("C4").SetValue("Stock initial (système)").Style.Font.SetBold(true);
                worksheet.Cell("D4").SetValue("Stock final (système)").Style.Font.SetBold(true);
                worksheet.Cell("E4").SetValue("Stock final (saisie manuelle)").Style.Font.SetBold(true);
                worksheet.Cell("F4").SetValue("Différence système").Style.Font.SetBold(true);
                worksheet.Cell("G4").SetValue("Différence manuelle").Style.Font.SetBold(true);
                worksheet.Cell("H4").SetValue("Différence de stock").Style.Font.SetBold(true);
                worksheet.Cell("I4").SetValue("Coût de l'article").Style.Font.SetBold(true);
                worksheet.Cell("J4").SetValue("Différence de coût").Style.Font.SetBold(true);
                ...
                if (items.Count > 0)
                {
                    currentCell.CellRight(8).SetValue("Écart de coût").Style.Font.SetBold(true);
